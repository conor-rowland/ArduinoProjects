...
void loop() {
  ...
  RainbowCycleMirror(th,1.0,0.2,1,ishift,mew,sigma,numblocks);
...
void RainbowCycleMirror(int H, float S, float LorV, int wait, int ishift, float mew, float sigma, int numblocks) {
  ...
  for(int i=0; i<blockLEDs; i++) {
    HSVtoRGB((H+int(360.0/LED_COUNT*i))%360,S,LorV,rgb1);
    // HSVtoRGB((H+int(360.0/LED_COUNT*(i+int(LED_COUNT/2))))%360,S,LorV,rgb2);
    float pixelfilt = 0;
    // pixelfilt = 0.95*exp(-abs(i-mew)/sigma) + 0.95*exp(-abs(-i-mew)/sigma);
    if (mew-4.5*sigma < i & i <= mew) {
      pixelfilt = 0.95*exp(-abs(i-mew)/sigma)+0.02;
    ...
    for(int b=0; b<numblocks; b++) {
      strip.setPixelColor(b*blockLEDs+(i+ishift)%blockLEDs, rgb1[0]*pixelfilt, rgb1[1]*pixelfilt, rgb1[2]*pixelfilt);
      // strip.setPixelColor(b*blockLEDs+(i+ishift)%blockLEDs, rgb1[0]*pixelfilt, rgb1[1]*pixelfilt, rgb1[2]*pixelfilt);
    }
    // strip.setPixelColor(int(LED_COUNT/2)+(i+ishift)%int(LED_COUNT/2), rgb2[0]*pixelfilt, rgb2[1]*pixelfilt, rgb2[2]*pixelfilt);
  }
  // for(int i=int(LED_COUNT/2); i<LED_COUNT; i++) {
  //   HSVtoRGB((H+int(360.0/LED_COUNT*i))%360,S,LorV,rgb1);
  //   float pixelfilt = 0;
  //   if (mew-4.5*sigma+int(LED_COUNT/2) < i & i <= mew+int(LED_COUNT/2)) {
  //     pixelfilt = 0.95*exp(-abs(i-int(LED_COUNT/2)-mew)/sigma)+0.02;
  //   }
  //   strip.setPixelColor(int(LED_COUNT/2)+(i+ishift)%int(LED_COUNT/2), rgb1[0]*pixelfilt, rgb1[1]*pixelfilt, rgb1[2]*pixelfilt);
  // }
