...
void loop() {
  ...
  // et = et+0.014;
  et = et+0.005*(0.75-0.5*randt/100.0);
  if (et > 0.99999999) {
    et = 0.0;
...
void RainbowCycle(float P, float B, int wait) {
  ...
  for(int i=0; i<31; i++) {
    RainbowCycle(fmodf(P+100.0/31*(i),100),B,rgb);
    strip.setPixelColor(i, rgb[0], rgb[1], rgb[2]);
  }
  for(int i=31; i<62; i++) {
    RainbowCycle(fmodf(P+100.0/31*(i-31),100),B,rgb);
    strip.setPixelColor(61-(i-31), rgb[0], rgb[1], rgb[2]);
  }
...
void RainbowCycle_Comet(float P, float B, float et, int wait) {
  ...
  for(int i=0; i<31; i++) {
    float ep = fmodf(i+0.5,LED_COUNT/4.0)/(LED_COUNT/4.0);
    CometEnvelope(&intensity,ep,2*et);
    float eB = B*intensity;
    RainbowCycle(fmodf(P+100.0/31*(i),100),eB,rgb);
    strip.setPixelColor(i, rgb[0], rgb[1], rgb[2]);
  }
  for(int i=31; i<62; i++) {
    float ep = fmodf(i,LED_COUNT/4.0)/(LED_COUNT/4.0);
    CometEnvelope(&intensity,ep,2*et);
    float eB = B*intensity;
    RainbowCycle(fmodf(P+100.0/31*(i-31),100),eB,rgb);
    strip.setPixelColor(61-(i-31), rgb[0], rgb[1], rgb[2]);
  }
...
void WashCycle(float etr, float etg, float etb, float etc, float etm, float ety, float B, int wait) {
  ...
  for(int i=0; i<LED_COUNT; i++) {
    float ep = fmodf(i,0.5*LED_COUNT)/(0.5*LED_COUNT);
